...
from database import create_connection, DATABASE_VISITS, DATABASE_REQUESTS, DATABASE_VISITS_REPORT
from utils import short_names, REPORTS_FOLDER, delete_old_reports
...
def generate_analytics_report():
    try:
        ...
        if conn_visits and conn_requests:
            ...
            report_df = pd.merge(visits_df, requests_df, on=["uid", "name", "student_group"], suffixes=("_visit", "_request"))
            ...
            report_df["Посещения по заявке"] = report_df.apply(
                lambda row: sum(1 for col in meal_columns if requests_df.loc[row.name, col] == 1 and row[col] == 1), axis=1)
            report_df["Посещения без заявки"] = report_df.apply(
                lambda row: sum(1 for col in meal_columns if requests_df.loc[row.name, col] == 0 and row[col] == 1), axis=1)
            report_df["Процент посещения"] = report_df.apply(
                lambda row: round((row["Посещения по заявке"] / row["Всего заявок"] * 100)) if row["Всего заявок"] > 0 else 0, axis=1)

            final_columns = ["name", "student_group"] + meal_columns + ["Всего заявок", "Посещения по заявке", "Посещения без заявки", "Процент посещения"]
            ...
            total_row.update({"Всего заявок": "", "Посещения по заявке": "", "Посещения без заявки": "", "Процент посещения": ""})
            final_report_df = pd.concat([final_report_df, pd.DataFrame([total_row])], ignore_index=True)

            settings = load_settings()
            min_percent = settings.get("min_percent", 65)
            from datetime import datetime
            current_date = datetime.now().strftime("%Y-%m-%d")
            default_filename = f"Отчет_с_аналитикой_{current_date}.xlsx"
            report_path = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")], initialfile=default_filename)
            if report_path:
                wb = Workbook()
                ws = wb.active
                ...
                ws.conditional_formatting.add(f"X2:X{ws.max_row - 1}", CellIsRule(operator='lessThan', formula=[min_percent], font=red_font))
