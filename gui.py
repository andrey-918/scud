...
from database import load_database, create_connection, DATABASE_VISITS, DATABASE_REQUESTS, DATABASE_VISITS_REPORT
from reports import generate_visits_report, generate_analytics_report
from settings import load_settings, save_settings
from utils import get_meal_type, is_saturday_dinner_end, get_last_report_date, get_last_saturday, save_last_report_date, delete_old_reports
...
def open_settings(root):
    ...
    def create_time_section(parent, meal_name, meal_data):
        ...
        ttk.Combobox(start_frame, textvariable=hour_var, values=[str(i).zfill(2) for i in range(24)], 
                     font=time_font, width=3, state='readonly').pack(side='left', padx=5)
        tk.Label(start_frame, text=":", font=label_font).pack(side='left')
        ttk.Combobox(start_frame, textvariable=minute_var, values=[str(i).zfill(2) for i in range(60)], 
                     font=time_font, width=3, state='readonly').pack(side='left', padx=5)
        ...
        ttk.Combobox(end_frame, textvariable=end_hour_var, values=[str(i).zfill(2) for i in range(24)], 
                     font=time_font, width=3, state='readonly').pack(side='left', padx=5)
        tk.Label(end_frame, text=":", font=label_font).pack(side='left')
        ttk.Combobox(end_frame, textvariable=end_minute_var, values=[str(i).zfill(2) for i in range(60)], 
                     font=time_font, width=3, state='readonly').pack(side='left', padx=5)
        
        return {'start': (hour_var, minute_var), 'end': (end_hour_var, end_minute_var)}
    ...
    tk.Button(button_frame, text="Настройка времени на ПК", command=lambda: open_rtc_time_setting(root), 
...
def open_rtc_time_setting(root):
    ...
    ttk.Combobox(content_frame, textvariable=year_var, values=[str(i) for i in range(2000, 2100)], 
    ...
    ttk.Combobox(content_frame, textvariable=month_var, values=[str(i).zfill(2) for i in range(1, 13)], 
                font=time_font, state='readonly').pack(fill='x', padx=20)
    ...
    ttk.Combobox(content_frame, textvariable=day_var, values=[str(i).zfill(2) for i in range(1, 32)], 
                font=time_font, state='readonly').pack(fill='x', padx=20)
    ...
    ttk.Combobox(content_frame, textvariable=hour_var, values=[str(i).zfill(2) for i in range(24)], 
                font=time_font, state='readonly').pack(fill='x', padx=20)
    ...
    ttk.Combobox(content_frame, textvariable=minute_var, values=[str(i).zfill(2) for i in range(60)], 
                font=time_font, state='readonly').pack(fill='x', padx=20)
    ...
    ttk.Combobox(content_frame, textvariable=second_var, values=[str(i).zfill(2) for i in range(60)], 
                font=time_font, state='readonly').pack(fill='x', padx=20)
    ...
    tk.Button(button_frame, text="Установить время", command=set_time, font=button_font, bg="#2196F3", fg="white").pack(fill='x', pady=5)
    tk.Button(button_frame, text="Выход", command=rtc_window.destroy, font=button_font, bg="#f44336", fg="white").pack(fill='x', pady=5)
...
def open_user_window(root):
    ...
    tk.Button(password_frame, text="Войти", command=check_password, font=button_font, bg="#2196F3", fg="white").pack(pady=20, ipady=20, fill='x')
    tk.Button(password_frame, text="Выход", command=password_window.destroy, font=button_font, bg="#f44336", fg="white").pack(pady=20, ipady=20, fill='x')
...
def show_user_window(root):
    ...
    tk.Button(middle_frame, text="Сформировать отчёт с аналитикой", command=generate_analytics_report, 
             font=button_font, bg="#2196F3", fg="white").pack(expand=True, fill='both', pady=10)
    tk.Button(middle_frame, text="Настройки", command=lambda: open_settings(root), 
             font=button_font, bg="#2196F3", fg="white").pack(expand=True, fill='both', pady=10)
    tk.Button(main_frame, text="Выход", command=user_window.destroy, 
